...
- **Staff+ (L6/E6+):** Drives the **RADIO framework**. Client bar: (a) **virtualized grid sync with markers**: react-window FixedSizeList/Grid renders ~10-50 visible of N; pure DOM cards beyond ~100 visible regress LCP/INP; canvas/WebGL marker layer for thousands; (b) **URL state via history.replaceState** (not pushState) so panning doesn't pollute back-stack but page remains shareable/bookmarkable; (c) **hover-link bidirectional sync** over shared selectedListingId in app state; debounce viewport-bbox refetch on pan/zoom (~300-500ms); (d) **two views over same ranked candidate set**: map pins and list cards must consume one backend response — they cannot diverge or the user sees ghost listings. **Backend context** (cite only — not the frontend bar): map-pin ranking differs from list-card ranking (pins have no attention-decay-by-position) [Airbnb Engineering]; neural location retrieval produces lat/lng offsets for relevant map area [Airbnb 2024]. These shape the API the client consumes but belong to §9 ml-in-loop, not this archetype.
