...
## (Caveat / spine note)
The 10M-cell cap, ~100-editor cap, and the Docs OT lineage are Google-confirmed; **OT-on-grid, the range-op transforms, and the dependency-DAG specifics are inferred** from the published Docs OT model + spreadsheet convention, not a Google primary source for Sheets — flag this in a mock. `google-sheets` extends `google-docs` (OT) onto a grid + dataflow graph; the CRDT alternative is discussed in `crdt-primitive`/`collaborative-text-editor`.
