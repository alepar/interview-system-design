...
**Fleet escalation.** The stateless gateway tier handles auth, **per-tenant token-quota** rate limiting (token-bucket per tenant_id, decremented as output tokens emit — not on request start), and request routing. A **cache-affinity-aware router** uses sticky-hash by `(tenant_id, prompt_prefix_hash)` to keep requests with the same system prompt landing on the same inference replica, maximizing prompt-cache hit rate. The **prompt-cache cluster** stores KV blocks keyed by `(content_hash, model_version, tenant_salt)` across multiple tiers: HBM (active, ~ms access), CPU DRAM (warm, ~10ms), SSD (cold, ~100ms). Cross-tenant isolation enforced via per-tenant `cache_salt` mixed into the cache key — prevents one tenant's prompt from being served from another's cached KV. The **metering pipeline** (Kafka stream of `(tenant_id, model, input_tokens, cached_input_tokens, output_tokens)` per request completion) feeds billing and quota refunds. Streaming responses use SSE with bounded per-connection buffers; backpressure on slow clients drops the connection rather than buffering unboundedly.
...
3. **Prompt cache topology and per-tenant isolation (fleet)** — Anthropic's published numbers (90% cost reduction, 85% latency reduction on cached prefixes) require a cluster-grade prompt-cache layer. The cached object is the KV state of the transformer for a prompt prefix — much larger than a typical response cache (tens of GB for a long system prompt on a 70B model). Topology options: (a) per-pod prefix tree (RadixAttention, SGLang) — each inference pod independently caches what passes through it, requires sticky routing to maximize hit rate; (b) shared cache cluster — separate fleet stores KV blocks, pods fetch on demand, introduces fetch latency on the hot path; (c) hybrid with per-pod hot tier + shared warm tier. Eviction: LRU at block granularity. Cross-tenant isolation: `cache_salt` per tenant mixed into the cache key — without it, tenant A could observe tenant B's cached system prompt by replaying the same prefix. Cross-version invalidation: cache key includes model version; during rolling deploy, drain old cache pool while warming new. Pricing math: Anthropic charges 1.25× base for 5-min cache writes, 0.10× base for reads, 2× base for 1-hour writes — break-even is ~3 reads per write, so caching is net-positive only for high-reuse prompts; surface a server-side decision to skip caching for low-reuse tenants. Staff+ commit: cache key shape, eviction policy, TTL strategy, deploy-time invalidation procedure.
...
- **Connection economics is the recurring gap** (per `state/observed.md` from the uber session). At 11.4M concurrent connections (ChatGPT scale), per-connection memory in the gateway tier drives pod sizing; the candidate should pin this in HLD, not deep-dive.
