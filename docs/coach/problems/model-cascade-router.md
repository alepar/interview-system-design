...
3. **MoE serving as intra-model routing.** Parallel routing problem to this archetype — but at the *intra-model* level (router selects top-K experts per token) rather than *inter-model* (router selects which model handles the request). For cascade-router purposes, an MoE flagship like DeepSeek-V3 is one **tier** in the cascade; the all-to-all collectives, expert-parallelism, routing-collapse defense, and 9.3–17.5% MoETuner speedup live in `moe-serving` (this archetype) — reference, don't re-derive. Staff+ commit here: when does it make sense to *also* cascade across MoE configurations (e.g., "MoE-32B vs MoE-671B as tiers") versus treating MoE as monolithic? Usually monolithic — MoE's value proposition is fast serving with fewer activated params, so layering cascade under it is double-counting.
